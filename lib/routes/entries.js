...
function getNextEntry(created_at, cb) {
  db.entry.find({created_at: {$gt: created_at}}, {_id: 1, created_at: 1}).limit(1, function(err, next_entry) {
...
exports.ymd = function(req, res) {
  ...
  db.entry.find(q, function(err, entries) {
    if(err) { throw err; }
    if(entries.length === 0) {
      res.send({"entries": []});
      return;
